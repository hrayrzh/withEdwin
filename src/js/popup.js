...
function createPopUp(country, populaton, confirmed, deaths, active, recovered) {
	document.getElementById("h2").innerHTML = `<h1>${country}</h1>`;
  document.getElementById("ttt").innerHTML = `<tr><td>populaton</td><td>${populaton}</td></tr><tr><td>confirmed</td><td>${confirmed}</td></tr><tr><td>deats</td><td>${deaths}</td></tr><tr><td>active</td><td>${active}</td></tr><tr><td>recovered</td><td>${recovered}</td></tr>`;
